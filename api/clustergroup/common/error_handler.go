...
func (e ErrorHandler) errorResponseFrom(err error) *pkgCommon.ErrorResponse {
	...
	} else if cgroup.IsClusterGroupAlreadyExistsError(err) || cgroup.IsNoReadyMembersError(err) || cgroup.IsInvalidClusterGroupCreateRequestError(err) || cgroup.IsClusterGroupUpdateRejectedError(err) {
		code = http.StatusBadRequest
	}
